...
// Service is a generic interface to define any type of service. It is a
// longterm running object that can fetch any configuration needed and is
// responsible for creating ProtocolInstance.
...
type NewServiceFunc func(r Router, o Overlay, path string) Service
...
func (s *serviceFactory) start(name string, r Router, o Overlay, path string) (Service, error) {
	var id ServiceID
	var ok bool
	if id, ok = s.translations[name]; !ok {
...
func newServiceStore(r Router, o Overlay) *serviceStore {
	...
	for _, id := range ids {
		...
		s, err := ServiceFactory.start(name, r, o, configName)
		if err != nil {
